...
public class FancyBackground : MonoBehaviour
{
    ...
    void LateUpdate()
    {
        int current = KiraSongCell.MostCenterdCellIndex;
        int last = current - 1;
        int next = current + 1;

        var position = KiraSongCell.MostCenterdCellPosition;
        var shiftedPosition = KiraSongCell.MostCenterdCellButShiftedPosition;

        var len = Mathf.Abs(shiftedPosition - position) * 2;

        position = (position - 0.5f) / len + 0.5f;

        KiraSongCell.MostCenterdCellPosition = -1;
        KiraSongCell.MostCenterdCellButShiftedPosition = -1;
